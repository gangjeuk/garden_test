...
# Reference
[[Obsidian_test/CTF/Basic/assets/ELF 파일 형식에서 재배치(Relocation), 링킹(Linking) 까지/ELF-64 Object File Format.pdf]]
[[Obsidian_test/CTF/Basic/assets/ELF 파일 형식에서 재배치(Relocation), 링킹(Linking) 까지/ELF-64 Object File Format, Version 1.4 1 ELF-64 Object File Format Including HP and HP-UX Extensions.pdf]]
...
![Obsidian_test/CTF/Basic/assets/ELF 파일 형식에서 재배치(Relocation), 링킹(Linking) 까지/image-20231005122731689.png](/img/user/Obsidian_test/CTF/Basic/assets/ELF%20%ED%8C%8C%EC%9D%BC%20%ED%98%95%EC%8B%9D%EC%97%90%EC%84%9C%20%EC%9E%AC%EB%B0%B0%EC%B9%98(Relocation),%20%EB%A7%81%ED%82%B9(Linking)%20%EA%B9%8C%EC%A7%80/image-20231005122731689.png)
...
![Obsidian_test/CTF/Basic/assets/ELF 파일 형식에서 재배치(Relocation), 링킹(Linking) 까지/image-20231006122304854.png](/img/user/Obsidian_test/CTF/Basic/assets/ELF%20%ED%8C%8C%EC%9D%BC%20%ED%98%95%EC%8B%9D%EC%97%90%EC%84%9C%20%EC%9E%AC%EB%B0%B0%EC%B9%98(Relocation),%20%EB%A7%81%ED%82%B9(Linking)%20%EA%B9%8C%EC%A7%80/image-20231006122304854.png)
...
여기서 자세히 보아야 하는 값은 앞으로 나올 [[Obsidian_test/CTF/Basic/ELF 파일 형식에서 재배치(Relocation), 링킹(Linking) 까지#Section header entry\|Section]]과 [[Obsidian_test/CTF/Basic/ELF 파일 형식에서 재배치(Relocation), 링킹(Linking) 까지#Program\|Program]]에서 다루어질 Section header, Program header의 위치를 가리키는 *e_phoff*, *e_shoff*이다.

각 헤더는 엔트리로 구성되고 다음과 같은 구조로 되어있다.

![Obsidian_test/CTF/Basic/assets/ELF 파일 형식에서 재배치(Relocation), 링킹(Linking) 까지/entries.png](/img/user/Obsidian_test/CTF/Basic/assets/ELF%20%ED%8C%8C%EC%9D%BC%20%ED%98%95%EC%8B%9D%EC%97%90%EC%84%9C%20%EC%9E%AC%EB%B0%B0%EC%B9%98(Relocation),%20%EB%A7%81%ED%82%B9(Linking)%20%EA%B9%8C%EC%A7%80/entries.png)
...
먼저 앞에서 설명한 [[Obsidian_test/CTF/Basic/ELF 파일 형식에서 재배치(Relocation), 링킹(Linking) 까지#Section header entry\|Section header entry]]를 다시 한번 상기하자.
...
![Obsidian_test/CTF/Basic/assets/ELF 파일 형식에서 재배치(Relocation), 링킹(Linking) 까지/image-20231006152734451.png](/img/user/Obsidian_test/CTF/Basic/assets/ELF%20%ED%8C%8C%EC%9D%BC%20%ED%98%95%EC%8B%9D%EC%97%90%EC%84%9C%20%EC%9E%AC%EB%B0%B0%EC%B9%98(Relocation),%20%EB%A7%81%ED%82%B9(Linking)%20%EA%B9%8C%EC%A7%80/image-20231006152734451.png)
...
![Obsidian_test/CTF/Basic/assets/ELF 파일 형식에서 재배치(Relocation), 링킹(Linking) 까지/image-20231006154233302.png](/img/user/Obsidian_test/CTF/Basic/assets/ELF%20%ED%8C%8C%EC%9D%BC%20%ED%98%95%EC%8B%9D%EC%97%90%EC%84%9C%20%EC%9E%AC%EB%B0%B0%EC%B9%98(Relocation),%20%EB%A7%81%ED%82%B9(Linking)%20%EA%B9%8C%EC%A7%80/image-20231006154233302.png)

실제 재배치에 필요한 정보를 가지는  Symbol Table Section 과 Relocation Section은 [[Obsidian_test/CTF/Basic/ELF 파일 형식에서 재배치(Relocation), 링킹(Linking) 까지#File header\|위와]]같이 각각 엔트리로 구성되어 있다. 

![Obsidian_test/CTF/Basic/assets/ELF 파일 형식에서 재배치(Relocation), 링킹(Linking) 까지/image-20231006184427053.png](/img/user/Obsidian_test/CTF/Basic/assets/ELF%20%ED%8C%8C%EC%9D%BC%20%ED%98%95%EC%8B%9D%EC%97%90%EC%84%9C%20%EC%9E%AC%EB%B0%B0%EC%B9%98(Relocation),%20%EB%A7%81%ED%82%B9(Linking)%20%EA%B9%8C%EC%A7%80/image-20231006184427053.png)
...
## Address Calculation
실제 주소의 계산은 [[Obsidian_test/CTF/Basic/ELF 파일 형식에서 재배치(Relocation), 링킹(Linking) 까지#Relocation Entry(.rela, .rel)\|Relocation Entry]]장에서 설명한  r_info를 이용해서 이루어진다.
...
- S: 재배치 후에 해당 심볼의 실제 위치
	- 계산식: ([[Obsidian_test/CTF/Basic/assets/ELF 파일 형식에서 재배치(Relocation), 링킹(Linking) 까지/st_value 의미\|st_value]]) + 심볼이 정의된 섹션이 로딩된 어드레스
- P: 재배치 해야하는 부분의 위치
...
## Procedure
사용 프로그램으로는 [[Obsidian_test/CTF/Basic/ELF 파일 형식에서 재배치(Relocation), 링킹(Linking) 까지#실습 준비\|2 번째 프로그램(Global)]]을 사용할 것이다.
...
전체적인 구조는 다음과 같다.
![Obsidian_test/CTF/Basic/assets/ELF 파일 형식에서 재배치(Relocation), 링킹(Linking) 까지/image-20231010151410269.png](/img/user/Obsidian_test/CTF/Basic/assets/ELF%20%ED%8C%8C%EC%9D%BC%20%ED%98%95%EC%8B%9D%EC%97%90%EC%84%9C%20%EC%9E%AC%EB%B0%B0%EC%B9%98(Relocation),%20%EB%A7%81%ED%82%B9(Linking)%20%EA%B9%8C%EC%A7%80/image-20231010151410269.png)

계산식은 다음과 같고
Address: <span class="green">S</span> + <span class="blue">A</span> - <span class="yellow">P</span>  
위에서 [[Obsidian_test/CTF/Basic/ELF 파일 형식에서 재배치(Relocation), 링킹(Linking) 까지#Address Calculation\|주어진 식]]을 조금 더 풀어 쓰면 다음과 같다.
$$
...
구조의 경우 [[Obsidian_test/CTF/Basic/ELF 파일 형식에서 재배치(Relocation), 링킹(Linking) 까지#Symbol Table Entry(.symtab)\|Symbol Table Entry]]와 동일한 구조를 가지고 구조체 또한 같은 구조체를 가진다.

Symbol Table Entry와 같이 그림으로 나타내면 다음과 같다.
![Obsidian_test/CTF/Basic/assets/ELF 파일 형식에서 재배치(Relocation), 링킹(Linking) 까지/image-20231025113003181.png](/img/user/Obsidian_test/CTF/Basic/assets/ELF%20%ED%8C%8C%EC%9D%BC%20%ED%98%95%EC%8B%9D%EC%97%90%EC%84%9C%20%EC%9E%AC%EB%B0%B0%EC%B9%98(Relocation),%20%EB%A7%81%ED%82%B9(Linking)%20%EA%B9%8C%EC%A7%80/image-20231025113003181.png)
...
자세한 내용은 [[Obsidian_test/CTF/Basic/assets/ELF 파일 형식에서 재배치(Relocation), 링킹(Linking) 까지/ELF64_Dyn 의미\|다음과 같다]].
...
자세한 내용은 [[Obsidian_test/CTF/Basic/GOT, PLT 부터 Dynamic Linking 까지\|Obsidian_test/CTF/Basic/GOT, PLT 부터 Dynamic Linking 까지]]에서 다룬다.
